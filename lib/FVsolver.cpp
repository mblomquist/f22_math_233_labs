...
void FVsolver::solve(std::vector<double> &solution) {
    ...
    for (int n = 0; n <= grid.get_nb_of_nodes(); n++) {
        ...
        if (grid.i_from_n(n) == 0)
            rhs[n] += (*boundary_term)(xmin, grid.y_from_n(n)) *
                      compute_length(ymax - ymin, (*levelset)(xmin, ymin), (*levelset)(xmin, ymax));
        ...
        if (grid.i_from_n(n) == grid.get_N() - 1)
            rhs[n] += (*boundary_term)(xmax, grid.y_from_n(n)) *
                      compute_length(ymax - ymin, (*levelset)(xmax, ymin), (*levelset)(xmax, ymax));
        else {
        ...
        if (grid.j_from_n(n) == 0)
            rhs[n] += (*boundary_term)(grid.x_from_n(n), ymin) *
                      compute_length(xmax - xmin, (*levelset)(xmin, ymin), (*levelset)(xmax, ymin));
        else {
        ...
        if (grid.j_from_n(n) == grid.get_M() - 1)
            rhs[n] += (*boundary_term)(grid.x_from_n(n), ymax) *
                      compute_length(xmax - xmin, (*levelset)(xmin, ymax), (*levelset)(xmax, ymax));
        else {
        ...
        rhs[n] +=
                (*boundary_term)(grid.x_from_n(n), grid.y_from_n(n)) * integrate_over_interface((*levelset)(xmin, ymin),
                                                                                                (*levelset)(xmax, ymin),
                                                                                                (*levelset)(xmin, ymax),
                                                                                                (*levelset)(xmax, ymax),
                                                                                                (xmax - xmin),
    ...
    std::cout << "Print rhs:" << std::endl;
    for (int i = 0; i < rhs.size(); i++){
        std::cout << rhs[i] << std::endl;
    }
    my_solver.solve(matrix, rhs, solution);
}
...
double
FVsolver::integrate_over_interface(double phi00, double phi10, double phi01, double phi11, double dx, double dy) {
